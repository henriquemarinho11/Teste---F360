...
namespace TesteLogica_F360
{
    public static class Iniciar
    {
        public static void Programa()
        {
            ...
            foreach (var p in palavrasTextoB)
            {
                if (p.Length == 3 && !foo.Contains(p.Last()) && !p.Select(p => p != 'd').Contains(false))
                {
                    Prep.Add(p);
                }
            }
            ...
            Console.WriteLine($"P: E no Texto B, quantas preposições existem?\nR: {Prep.Count()}\n\n---\n");
            ...
            List<string> verbosPP = new List<string>();

            foreach (var p in palavrasTextoB)
            {
                if (p.Length > 7 && foo.Contains(p.Last()))
                {
                    verbos.Add(p);

                    if (!foo.Contains(p.First()))
                    {
                        verbosPP.Add(p);
                    }
                }
            ...
            Console.WriteLine($"P: Já no Texto B, quantos são os verbos?\nR: {verbos.Count()}\n\n---\n");

            Console.WriteLine("Exercicio 3\n");
            Console.WriteLine($"P: E quantos verbos do Texto B estão em primeira pessoa?\nR: {verbosPP.Count()}\n\n---\n");
            ...
            List<string> vocabularioTextoB = new List<string>();

            vocabularioTextoB = palavrasTextoB.Distinct().ToList();

            vocabularioTextoB = vocabularioTextoB.OrderBy(i => AlfabetoKlingon.IndexOf(i[0]))
                                                 .ThenBy(i => AlfabetoKlingon.IndexOf(i[1]))
                                                 .ThenBy(i => AlfabetoKlingon.IndexOf(i[2])).ToList();
           
            Console.WriteLine("Exercicio 4\n");
            Console.Write("P: Como seria a lista de vocabulário do Texto B?\nR: ");
            
            foreach (var p in vocabularioTextoB)
            {
                Console.Write(p + " ");
            }
            ...
            foreach (var p in vocabularioTextoB)
            {
                ...
                {
                    NumPalavra += AlfabetoKlingon.IndexOf(p[x]) * Math.Pow(20, x);
                }
            ...
            Console.WriteLine("Exercicio 4\n");
            Console.WriteLine($"P: E no Texto B, quantos números bonitos distintos existem?\nR: {NumBonitos}\n");
            Console.ReadKey();
